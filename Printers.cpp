...
namespace Printers
{
    void PrintHeader(File& file, const std::string& fileName, const std::string& fileExtension, bool pragmaPackPush)
    {
        ...
        file.WriteLine("# Generated with the UE3SDKGenerator " + Engine::GeneratorVersion);
        file.WriteLine("# ========================================================================================= #");
        file.WriteLine("# File: " + fileName + "." + fileExtension);
        file.WriteLine("# ========================================================================================= #");
        file.WriteLine("# Credits: " + Engine::GeneratorCredits);
        file.WriteLine("# Links: " + Engine::GeneratorLinks);
        file.WriteLine("#############################################################################################");
        file.WriteLine("*/");
        ...
        if (fileName != "SdkHeaders")
        {
            if (fileExtension == "hpp" && fileName != "GameDefines")
            {
                ...
                {
                    file.WriteLine("#include \"../SdkConstants.hpp\"");
                }
            }
            else if (fileExtension == "cpp" && fileName != "GameDefines")
            {
                file.WriteLine("#include \"../SdkHeaders.hpp\"");
            }
        }
        ...
        if (pragmaPackPush)
        {
            ...
            file.WriteLine("\t#pragma pack(push, 0x" + std::to_string(static_cast<uint8_t>(Configuration::Alignment)) + ")");
